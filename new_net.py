...
class Generation_net(nn.Module):
    def __init__(self):
        ...
        self.inception1 = InceptionGen(1024, 384, 192, 384, 48, 128, 128)
        # # 512
        self.deconv2 = BasicConvTranspose3d(1024, 512, kernel_size=4, stride=2, padding=1)
        self.inception2 = InceptionGen(512, 128, 128, 256, 24, 64, 64)
        # # 256
        self.deconv3 = BasicConvTranspose3d(512, 256, kernel_size=4, stride=2, padding=1)
        self.inception3 = InceptionGen(256, 64, 96, 128, 16, 32, 32)
        # # 256-128-64-64
...
def gnerate():
    return Generation_net()


model = Generation_net()
model.cuda()
summary(model, (4096, 4, 4))
